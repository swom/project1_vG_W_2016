...
namespace glsl
{
                                  ...
                                  GLsizei length, const GLchar* message, const GLvoid*)
  {
    ...
    std::fprintf(stderr, "\nOpenGL log (%d):  Type: %s  Source: %s  Severity: %s\n%s\n", 
                 id, getStringForType(type), getStringForSource(source), getStringForSeverity(severity), message);
  }
  ...
  void __stdcall GLDebugLogOnceStdErrThrow(GLenum source, GLenum type, GLuint id, GLenum severity,
    GLsizei length, const GLchar* message, const GLvoid*)
  {
    GLDebugLogOnceStdErr(source, type, id, severity, length, message, nullptr);
    if (severity == GL_DEBUG_SEVERITY_HIGH) throw std::runtime_error("unrecoverable OpenGL error");
  }
