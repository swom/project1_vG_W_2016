...
blue <- rgb(0, 0, 255, max = 255, alpha = 0.5, names = "o")
...
for (i in levels(all_reac_norms$seed)) {
  for (j in levels(all_reac_norms$change)) {
    ...
    reac_norm = all_reac_norms %>% subset(seed == i & change == j  )
    plot3d(x = "Energy", y = "Metabolite", z = "Food",
           xlab = "Energy", ylab = "Metabolite", zlab = "Food",
           box = TRUE,
           type ="p", axis.scale = FALSE,
           xlim = c(0,20), ylim = c(0,20), zlim = c(0,20)
           )
